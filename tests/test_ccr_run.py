...
class TestCCRRun(unittest.TestCase):
    ...
    def test_dry_run_end_to_end_writes_expected_artifacts(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            review_prepare = json.loads(Path(manifest["review_prepare_file"]).read_text(encoding="utf-8"))
            ...
            self.assertEqual(review_prepare["contract_version"], "ccr.review_prepare.v1")
            self.assertGreaterEqual(review_prepare["summary"]["requirement_clause_count"], 1)
            self.assertGreaterEqual(review_prepare["summary"]["dimension_count"], 1)
            self.assertTrue(review_prepare["questions_to_verify"])
            ...
            self.assertEqual(status["stages"]["review_prepare"]["case_count"], review_prepare["summary"]["case_count"])
            ...
            self.assertEqual(summary["review_prepare_file"], manifest["review_prepare_file"])
    ...
    def test_detach_launch_and_watch_flow(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertTrue(str(launch["review_prepare_file"]).endswith("review_prepare.json"))
            ...
            self.assertEqual(summary["contract_version"], "ccr.run_summary.v1")
            self.assertEqual(run_metrics["contract_version"], "ccr.run_metrics.v1")
            review_prepare = json.loads(Path(launch["review_prepare_file"]).read_text(encoding="utf-8"))
            self.assertEqual(review_prepare["contract_version"], "ccr.review_prepare.v1")
            self.assertEqual(run_metrics["llm"]["total_calls"], 14)
