...
- Phase 4 Step 5 is complete: repo-local eval fixture suites now live under `evals/ccr/`, with deterministic local execution through `quality/scripts/ccr_eval.py` and `scripts/evals.sh`
- Phase 4 is complete for the planned non-CI rollout: local observability, run metrics, posting observability, and offline eval substrate are in place; CI integration remains explicitly deferred
